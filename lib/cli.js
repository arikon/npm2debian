...
exports.main = function () {
    ...
    require('coa').Cmd()
        ...
        .arg()
            ...
            .end()
        .opt()
            .name('registry').title('npm registry address')
            .long('registry')
            .end()
        .completable()
        .act(function(opts, args) {

            return loadConf({cache: PATH.resolve('.cache'), registry: opts.registry})
                .then(function() {
...
var loadConf = function(conf) {
    ...
    NPM.load(conf, function(err) {
        ...
        if(conf.registry) {
            NPM.config.set('registry', conf.registry); // npm doesn't set registry from conf
        }
        d.resolve();
    });
    return d.promise;
...
var mkdir = function(ensure, mode, uid, gid, noChmod) {
    var d = Q.defer();
    require('mkdir-p')(ensure, mode || '0777', uid, gid, noChmod, function(err) {
        err? d.reject(err) : d.resolve();
    });
    return d.promise;
};
